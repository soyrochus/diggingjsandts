...
async function main(rootDir: string){
    ...
    for await (const [name, stat] of async_stream(getFiles(rootDir)).filter(isDir).all() ){
        console.log(name, stat.isFile(), stat.size)  
    }
    console.log("__________________________________________________________")
    ...
    for await (const [name, stat] of async_stream(getFiles(rootDir)).filter(isDir).take(5) ){
        console.log(name, stat.isFile(), stat.size)  
    }
    console.log("__________________________________________________________")
    ...
    for await (const [name, stat] of async_stream(getFiles(rootDir)).filter(isDir).map(upper).take(5) ){
        console.log(name, stat.isFile(), stat.size)  
    }
}
